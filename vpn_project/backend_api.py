...
CORS(app,
     resources={r"/*": {
         "origins": [
             # Origen de Vue/Vite (cambia si usas otro puerto)
             "http://localhost:5173",
             "http://127.0.0.1:5173"
         ],
         # 🚨 CLAVE: Permitir X-API-Key
...
def control_tailscale(action):
    ...
    try:
        ...
        if "command not found" in error_message or "is not recognized" in error_message:
            raise Exception(
                f"Tailscale no está disponible en la ruta (PATH). Verifique la instalación o la ruta: {TAILSCALE_EXECUTABLE}")
